...
/* If the user chooses 'p' or 'P', show the list, else show an error if the list is empty */
void print_numbers( std::vector <double> const userVector ) {
	if ( userVector.size() ) {
		std::cout << "[ " ;
...
/* If the user chooses 'a' or 'A', allow them to add a number to the list. Display an error if it is not a number. */
void add_number( std::vector <double> &userVector ) {
	double user_num {} ;
...
/* If the user chooses an option not in the list, display an error */
void display_error( void ) {
	std::cerr << "Invalid -- try again!" << std::endl ;
}

/* If the user chooses 'q' or 'Q', show a quit message */
void exit_program( void ) {
	std::cout << "\nGoodbye!" << std::endl ;
}
